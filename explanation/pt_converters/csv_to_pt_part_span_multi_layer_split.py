#!/usr/bin/env python3
"""
Convert bbox-abduction CSV files to one PT file per extraction layer.

This shares each multimodal forward across multiple extraction-layer hooks, then
writes separate layer_<id>/<split>.pt files so the existing per-layer training
pipeline can consume the outputs unchanged.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Convert row-level bbox-abduction CSV to per-layer PT files using shared multimodal forwards."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="If >0, cap the number of PT entries with the same exact target JSON per layer.",
    )
    ...
    parser.add_argument(
        "--image-cache-dir",
        default="",
        help="Shared directory used to store resized images referenced by all per-layer PT files.",
    )
    parser.add_argument(
        "--image-jpeg-quality",
        type=int,
    ...
    if max_entries_per_target is not None:
        print(f"Max entries per exact target per layer: {max_entries_per_target}", file=sys.stderr)
    ...
    print(
        ...
        f"bbox spans kept: {kept_segments}, per-layer PT entries written: {counts_preview}",
        file=sys.stderr,
    )
