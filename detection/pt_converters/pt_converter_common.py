...
try:
    ...
    from nl_probes.utils.dataset_utils import create_training_datapoint  # noqa: E402
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            if str(getattr(config, "model_type", "") or "").lower() == "mllama":
        ...
        candidate_tokenizer = getattr(processor, "tokenizer", None)
        if callable(candidate_tokenizer):
            tokenizer = candidate_tokenizer
        elif callable(processor):
            if candidate_tokenizer is not None:
                print(
                    "  AutoProcessor.tokenizer is not callable; using the processor object itself as tokenizer.",
                    file=sys.stderr,
                )
            tokenizer = processor
...
def register_forward_hooks(model, layer_paths: list[str], layer_id_by_path: dict[str, int], get_target_module_fn):
    ...
    def _make_hook(layer_key: str):
        def hook_fn(module, inp, out):
            captured[layer_key] = (out[0] if isinstance(out, tuple) else out).detach().float().cpu()
