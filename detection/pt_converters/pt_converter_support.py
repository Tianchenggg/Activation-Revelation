...
def _apply_chat_template(tokenizer, conversation: list[dict[str, Any]], add_generation_prompt: bool) -> str:
    family = _infer_chat_template_family(tokenizer)
    if family == "chatglm":
        _maybe_patch_chatglm_chat_template_tokenizer(tokenizer)
        conversation = _normalize_conversation_for_chatglm(conversation)
...
def _infer_chat_template_family(tokenizer) -> str:
    class_name = type(tokenizer).__name__.lower()
    module_name = type(tokenizer).__module__.lower()
    name_or_path = str(getattr(tokenizer, "name_or_path", "") or "").lower()
    if "chatglm" in class_name or "chatglm" in module_name or "glm" in name_or_path:
        return "chatglm"
    return "generic"


def _maybe_patch_chatglm_chat_template_tokenizer(tokenizer) -> None:
    if _infer_chat_template_family(tokenizer) != "chatglm":
        return
    if not hasattr(tokenizer, "batch_encode_plus"):
        tokenizer.batch_encode_plus = tokenizer.__call__


def _normalize_conversation_for_chatglm(
    ...
    *,
    ...
    for item in conversation:
        ...
        normalized_item: dict[str, Any] = {
            "role": role,
            "content": _extract_text_only(item.get("content", "")),
        }
        metadata = item.get("metadata")
        if metadata not in (None, ""):
            normalized_item["metadata"] = str(metadata)
        tools = item.get("tools")
        if tools:
            normalized_item["tools"] = tools
        if image is not None and role == "user" and not image_attached:
...
def _build_multimodal_processor_output(
    *,
    processor,
    ...
    if _infer_chat_template_family(processor) == "chatglm":
        if conversation is None:
            raise ValueError("ChatGLM multimodal processing requires the original conversation.")
        _maybe_patch_chatglm_chat_template_tokenizer(processor)
        normalized_conversation = _normalize_conversation_for_chatglm(conversation, image=pil_image)
...
def _approximate_offsets_from_input_ids(
    tokenizer,
    full_text: str,
    input_ids: list[int],
    special_ids: set[int],
    ...
    for token_id in input_ids:
        ...
        if start < 0 and full_text.startswith(piece, cursor):
            start = cursor
        if start < 0:
            start = cursor
        end = min(text_length, start + len(piece))
        if end < start:
            end = start
...
def infer_model_family(model_path: str, config: Any | None = None) -> str:
    if config is None:
    ...
    if "qwen3-vl" in model_path_lower or model_type == "qwen3_vl" or any("qwen3vl" in arch for arch in architectures):
        return "qwen3_vl"
    ...
    if model_type == "mllama" or any("mllama" in arch for arch in architectures):
        return "mllama"
    if "qwen3" in model_path_lower or model_type == "qwen3" or any("qwen3" in arch for arch in architectures):
        return "qwen3"
...
def get_text_layer_count(model_path: str, config: Any | None = None) -> int:
    if config is None:
        config = load_model_config(model_path)
    ...
    raise AttributeError(f"Could not infer language-layer count for {model_path}")


def layer_percent_to_layer(model_path: str, layer_percent: int, config: Any | None = None) -> int:
    max_layers = get_text_layer_count(model_path, config=config)
    layer = int(max_layers * (layer_percent / 100))
    return min(max(layer, 0), max_layers - 1)


def build_layer_path_for_component(
    model_path: str,
    layer_id: int,
    *,
    component: str = "block",
    ...
    family = infer_model_family(model_path, config=config)
    component_key = component.strip().lower()

    if family in {"qwen3_vl", "mllama"}:
        base_path = f"model.language_model.layers.{layer_id}"
        suffix_map = {
            "block": "",
            "self_attn": ".self_attn",
            "mlp": ".mlp",
        ...
        base_path = f"transformer.encoder.layers.{layer_id}"
        suffix_map = {
            "block": "",
        ...
        base_path = f"model.layers.{layer_id}"
        suffix_map = {
            "block": "",
            "self_attn": ".self_attn",
            "mlp": ".mlp",
    ...
    if component_key not in suffix_map:
        supported = ", ".join(sorted(suffix_map))
        raise ValueError(f"Unsupported component '{component}'. Supported: {supported}")
    return base_path + suffix_map[component_key]
...
def _get_model_loader_classes(model_path: str) -> list[Any]:
    ...
    try:
        ...
        if isinstance(auto_map, dict):
            for auto_key in ("AutoModelForCausalLM", "AutoModel"):
                class_ref = auto_map.get(auto_key)
                if isinstance(class_ref, str) and class_ref.strip():
                    try:
...
def load_model_with_fallback(model_path: str, torch_dtype: Any, device: str) -> torch.nn.Module:
    ...
    for model_cls in _get_model_loader_classes(model_path):
        ...
        try:
            direct_kwargs = dict(load_kwargs)
            if getattr(model_cls, "__module__", "").startswith("transformers_modules."):
                direct_kwargs.pop("trust_remote_code", None)
            if not hasattr(model_cls, "all_tied_weights_keys"):
                model_cls.all_tied_weights_keys = {}
            model = model_cls.from_pretrained(model_path, **direct_kwargs)
            if device != "auto":
...
def _build_text_and_token_state(
    *,
    ...
    tok_kwargs = dict(
        ...
        add_special_tokens=_infer_chat_template_family(tokenizer) != "chatglm",
    )

    enc_full = tokenizer(full_text, return_offsets_mapping=True, return_tensors="pt", **tok_kwargs)
    enc_prompt = tokenizer(prompt_only_text, return_tensors="pt", **tok_kwargs)

    text_input_ids: list[int] = enc_full["input_ids"][0].tolist()
    completion_start_tok = len(enc_prompt["input_ids"][0])
    special_ids = set(getattr(tokenizer, "all_special_ids", []) or [])
    if "offset_mapping" in enc_full:
        offsets: list[tuple[int, int]] = enc_full["offset_mapping"][0].tolist()
    else:
        offsets = _approximate_offsets_from_input_ids(
            tokenizer=tokenizer,
            full_text=full_text,
...
def _build_pos_map(
    *,
    ...
    try:
        proc_out = _build_multimodal_processor_output(
            processor=processor,
        ...
        if len(pos_map) != len(text_input_ids):
            missing_count = len(text_input_ids) - len(pos_map)
            mapped_indices = sorted(pos_map.keys())
            preview_missing: list[int] = []
            next_expected = 0
            for mapped_idx in mapped_indices:
                while next_expected < mapped_idx and len(preview_missing) < 16:
                    preview_missing.append(next_expected)
                    next_expected += 1
                next_expected = mapped_idx + 1
            while next_expected < len(text_input_ids) and len(preview_missing) < 16:
                preview_missing.append(next_expected)
                next_expected += 1

            msg = (
                f"[row {row_idx}] multimodal token remap for image '{image_path}' only covered "
                f"{len(pos_map)}/{len(text_input_ids)} text tokens "
                f"(multimodal_len={len(new_input_ids)}, missing={missing_count}, "
                f"sample_missing_indices={preview_missing})."
            )
            if require_image_processor:
                raise ImageProcessorRequiredError(msg)
            print(f"{msg} Falling back to text-only.", file=sys.stderr)
            return {i: i for i in range(len(text_input_ids))}
    except Exception as exc:
        if require_image_processor:
...
def forward_row_hidden(
    *,
    ...
    conversation_full = row_info.get("conversation_full")
    ...
    if image_path is not None and processor is not None:
        try:
            ...
            model_inputs = {key: value.to(device) for key, value in proc_out.items()}
        except Exception as exc:
            if require_image_processor:
                raise ImageProcessorRequiredError(
    ...
    if model_inputs is None:
        enc_full = tokenizer(full_text, return_tensors="pt", **tok_kwargs)
        model_inputs = {
