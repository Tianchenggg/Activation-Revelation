...
def _get_sequence_value(values: Any, index: int, default: Any = None) -> Any:
    if values is None:
        return default
    try:
        return values[index]
...
def _count_supervised_labels_from_packed(labels: Any, index: int) -> int | None:
    if not isinstance(labels, Mapping):
        return None
    offsets = labels.get("offsets")
    values = labels.get("values")
    if offsets is None or values is None:
        return None

    start = _maybe_item(_get_sequence_value(offsets, index))
    end = _maybe_item(_get_sequence_value(offsets, index + 1))
    if start is None or end is None:
        return None
    start = int(start)
    end = int(end)
    label_slice = values[start:end]
    if hasattr(label_slice, "ne"):
        return int(label_slice.ne(-100).sum().item())
    return sum(1 for label_id in label_slice if int(label_id) != -100)


def _get_target_parent_category_from_meta(meta_info: Any) -> str | None:
    if not isinstance(meta_info, Mapping):
        return None

    target_tag = meta_info.get("target_tag")
    if target_tag is not None:
        target_tag_parent = _extract_closed_tag_parent_category(str(target_tag))
        if target_tag_parent is not None:
            return target_tag_parent

    parent_category = meta_info.get("parent_category")
    if parent_category is not None:
        parent_category_text = str(parent_category).strip()
        if parent_category_text:
            return parent_category_text
...
def _get_lazy_packed_dataset_view(training_data: Any) -> tuple[Any, list[int] | None] | None:
    base_dataset = getattr(training_data, "base_dataset", None)
    indices = getattr(training_data, "indices", None)
    ...
    if not hasattr(training_data, "_shard_specs") or not hasattr(training_data, "_shard_start_indices"):
        return None
    if not callable(getattr(training_data, "_load_shard", None)):
        return None
    return training_data, indices


def _summarize_lazy_packed_training_dataset(training_data: Any) -> TrainingDatasetSummary | None:
    dataset_view = _get_lazy_packed_dataset_view(training_data)
    if dataset_view is None:
        return None

    base_dataset, indices = dataset_view
    shard_specs = list(getattr(base_dataset, "_shard_specs"))
    shard_start_indices = list(getattr(base_dataset, "_shard_start_indices"))
    if not shard_specs or len(shard_specs) != len(shard_start_indices):
        return None

    selected_by_shard: list[list[int] | None]
    if indices is None:
        selected_by_shard = [None for _ in shard_specs]
    else:
        selected_by_shard = [[] for _ in shard_specs]
        for global_index in indices:
            shard_index = bisect_right(shard_start_indices, global_index) - 1
            if shard_index < 0 or shard_index >= len(shard_specs):
                return None
            local_index = int(global_index) - int(shard_start_indices[shard_index])
            if local_index < 0 or local_index >= int(shard_specs[shard_index].num_entries):
                return None
            selected_by_shard[shard_index].append(local_index)
    ...
    for shard_index, shard_spec in enumerate(shard_specs):
        local_indices = selected_by_shard[shard_index]
        if local_indices == []:
            continue

        shard_obj = base_dataset._load_shard(shard_index)
        if not isinstance(shard_obj, Mapping) or shard_obj.get("format") != "ao_custom_pt_shard_v2":
            return None

        num_entries = min(int(shard_obj.get("num_entries", shard_spec.num_entries)), int(shard_spec.num_entries))
        iterable_indices = range(num_entries) if local_indices is None else local_indices

        layers = shard_obj.get("layer")
        target_outputs = shard_obj.get("target_output")
        labels = shard_obj.get("labels")
        meta_infos = shard_obj.get("meta_info")

        for local_index in iterable_indices:
            if local_index < 0 or local_index >= num_entries:
                return None

            layer_value = _get_sequence_value(layers, local_index)
            if layer_value is not None:
                layer_counts[int(_maybe_item(layer_value))] += 1

            target_output = str(_get_sequence_value(target_outputs, local_index, ""))
            answer_text = normalize_reasoning_answer_text(target_output)
            answer_text_counts[answer_text or target_output.strip()] += 1

            supervised_len = _count_supervised_labels_from_packed(labels, local_index)
            if supervised_len is not None:
                supervised_len_counts[supervised_len] += 1

            safety_label = infer_safety_label(target_output)
            if safety_label is None:
                continue
            safety_label_counts[safety_label] += 1
            if safety_label == UNSAFE_LABEL:
                parent_category = _get_target_parent_category_from_meta(
                    _get_sequence_value(meta_infos, local_index, {})
                )
                if parent_category is not None:
...
def summarize_training_dataset(training_data: Any) -> TrainingDatasetSummary:
    fast_summary = _summarize_lazy_packed_training_dataset(training_data)
