...
def get_hf_activation_steering_hook(
    ...
    def hook_fn(module, _input, output):
        nonlocal warned_non_finite, warned_outlier
        ...
        for b in range(B):
            ...
            if pos_b.min() < 0 or pos_b.max() >= L:
                pos_cpu = pos_b.detach().cpu()
                raise ValueError(
                    "Steering positions out of range for current sequence. "
                    f"batch_index={b}, seq_len={L}, num_positions={int(pos_b.numel())}, "
                    f"min_position={int(pos_cpu.min().item())}, max_position={int(pos_cpu.max().item())}, "
                    f"sample_positions={pos_cpu[:32].tolist()}"
                )
            if normed_list[b].shape[0] != pos_b.shape[0]:
                raise ValueError(
                    "Steering vector count does not match steering position count. "
                    f"batch_index={b}, vectors={int(normed_list[b].shape[0])}, positions={int(pos_b.shape[0])}"
                )
            if normed_list[b].shape[1] != d_model_actual:
                raise ValueError(
                    "Steering vector hidden size does not match hooked residual stream hidden size. "
                    f"batch_index={b}, vector_hidden={int(normed_list[b].shape[1])}, residual_hidden={d_model_actual}"
                )
            ...
            if b == 0:
                ...
                if not torch.isfinite(norms_flat).all():
                    if not warned_non_finite:
                        print("WARNING: Non-finite activation norm detected in steering hook.")
            ...
            if steering_mode == "replace":
                ...
                resid_BLD[b, pos_b, :] = orig_KD + steered_KD.detach()
