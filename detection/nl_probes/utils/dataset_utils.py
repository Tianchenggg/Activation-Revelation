...
def normalize_chat_template_token_ids(token_ids: Any, field_name: str) -> list[int]:
    """Normalize tokenizer/processor outputs into a single list[int]."""
    original = token_ids
    ...
    if not isinstance(token_ids, list) and hasattr(token_ids, "tolist"):
...
def get_prompt_tokens_only(
    ...
    """Return the prompt prefix that precedes the first supervised answer token."""
    first_supervised_idx = get_first_supervised_idx(training_data_point)

    if first_supervised_idx is None:
        raise ValueError("No supervised tokens found in training_data_point.labels")

    prompt_tokens = training_data_point.input_ids[:first_supervised_idx]
    prompt_labels = training_data_point.labels[:first_supervised_idx]
...
def get_first_supervised_idx(training_data_point: TrainingDataPoint) -> int | None:
    for i, label in enumerate(training_data_point.labels):
        if label != -100:
            return i
    return None


def _looks_like_qwen3_name(value: str | None) -> bool:
    return isinstance(value, str) and "qwen3" in value.lower()


@lru_cache(maxsize=128)
def _is_local_qwen3_model_dir(model_name: str) -> bool:
    ...
    if isinstance(architectures, list):
        return any(_looks_like_qwen3_name(str(architecture)) for architecture in architectures)
    return False


def is_qwen3_reasoning_model(model_name: str, tokenizer: AutoTokenizer | None = None) -> bool:
    if _looks_like_qwen3_name(model_name):
        return True
    if tokenizer is not None and _looks_like_qwen3_name(getattr(tokenizer, "name_or_path", None)):
        return True
    return _is_local_qwen3_model_dir(model_name)


def get_disable_thinking_prefix_ids(tokenizer: AutoTokenizer) -> list[int]:
    ...
    with_thinking_ids = normalize_chat_template_token_ids(with_thinking, "with_thinking_prompt_ids")
    without_thinking_ids = normalize_chat_template_token_ids(without_thinking, "without_thinking_prompt_ids")
    if not without_thinking_ids[: len(with_thinking_ids)] == with_thinking_ids:
        raise ValueError("Expected disable-thinking chat template ids to extend the standard generation prompt.")
    return without_thinking_ids[len(with_thinking_ids) :]


def maybe_add_disable_thinking_prefix(
    training_data_point: TrainingDataPoint,
    tokenizer: AutoTokenizer,
    model_name: str,
    ...
    existing_prefix = training_data_point.input_ids[
        max(0, first_supervised_idx - len(prefix_ids)) : first_supervised_idx
    ]
    if existing_prefix == prefix_ids:
        return training_data_point

    updated = training_data_point.model_copy(deep=False)
    updated.input_ids = (
        training_data_point.input_ids[:first_supervised_idx]
        + prefix_ids
        + training_data_point.input_ids[first_supervised_idx:]
    )
    updated.labels = (
        training_data_point.labels[:first_supervised_idx]
        + ([-100] * len(prefix_ids))
        + training_data_point.labels[first_supervised_idx:]
    )
    return updated
...
def maybe_add_disable_thinking_prefix_to_dataset(
    dataset: list[TrainingDataPoint],
    tokenizer: AutoTokenizer,
    model_name: str,
    ...
    prefix_ids = get_disable_thinking_prefix_ids(tokenizer)
    updated_dataset: list[TrainingDataPoint] = []
    num_modified = 0
    for data_point in dataset:
        updated_point = maybe_add_disable_thinking_prefix(data_point, tokenizer, model_name, prefix_ids)
        if updated_point != data_point:
            num_modified += 1
        updated_dataset.append(updated_point)
    return updated_dataset, num_modified
...
def materialize_missing_steering_vectors(
    ...
    use_lora_paths = isinstance(model, PeftModel)
    submodules = {layer: get_hf_submodule(model, layer, use_lora=use_lora_paths) for layer in layers_needed}

    # Run a single pass with dropout off, then restore the previous train/eval mode
    was_training = model.training
    model.eval()
    adapter_ctx = model.disable_adapter() if use_lora_paths else contextlib.nullcontext()
    with adapter_ctx:
...
def create_training_datapoint(
    ...
    input_prompt_ids = normalize_chat_template_token_ids(input_prompt_ids, "input_prompt_ids")
    ...
    full_prompt_ids = normalize_chat_template_token_ids(full_prompt_ids, "full_prompt_ids")

    assistant_start_idx = len(input_prompt_ids)

    # Only supervise the assistant answer content, not the templated turn terminator
    # (for example `<|im_end|>\n`), so training aligns with answer-level evaluation.
    empty_assistant_ids = tokenizer.apply_chat_template(
        input_messages + [{"role": "assistant", "content": ""}],
        tokenize=True,
        add_generation_prompt=False,
        return_tensors=None,
        padding=False,
    ...
    empty_assistant_ids = normalize_chat_template_token_ids(empty_assistant_ids, "empty_assistant_ids")

    full_suffix = full_prompt_ids[assistant_start_idx:]
    empty_suffix = empty_assistant_ids[assistant_start_idx:]
    common_suffix_len = 0
    max_common = min(len(full_suffix), len(empty_suffix))
    while common_suffix_len < max_common and full_suffix[-1 - common_suffix_len] == empty_suffix[-1 - common_suffix_len]:
        common_suffix_len += 1

    response_end_idx = len(full_prompt_ids) - common_suffix_len
